...
class Index extends Admin
{
    ...
    public function index()
    {
        ...
        return ZBuilder::make('table')
            ...
            ->setPageTips('<p style="color:#f3b760;"><strong><i class="fa fa-fw fa-info-circle"></i> 提示：</strong>ID为1和2的用户不能进行删除、禁用和启用操作。</p>') // 设置页面提示
            ...
            ->addColumns([ // 批量添加列
                ...
                ['username', '用户名'],
                ['nickname', '昵称'],
                ['role', '角色', 'callback','array_v', RoleModel::getTree(null, false)],
                ['email', '邮箱'],
                ['mobile', '手机号'],
    ...
    public function add()
    {
        ...
        $tree=RoleModel::getTree(null, false);
        if(UID!=1){
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            $data = $this->request->post();
            $info = UserModel::where('id', $data['id'])->field('password', true)->find();
            $data['role']=$info['role'];
    ...
    public function delete($ids = [])
    {
        ...
        $rt=db('admin_user')->where('id','in',$ids)->delete();
        if($rt!==false){
    ...
    public function enable($ids = [])
    {
        ...
        $rt=db('admin_user')->where('id','in',$ids)->update(['status'=>'1']);
        if($rt!==false){
            return $this->success('操作成功');
        }else{
            return $this->error('操作失败，请重试');
    ...
    {   
        ...
        $rt=db('admin_user')->where('id','in',$ids)->update(['status'=>'0']);
        if($rt!==false){
            return $this->success('操作成功');
        }else{
            return $this->error('操作失败，请重试');
