...
class Role extends Admin
{
    ...
    public function index()
    {
        ...
        return ZBuilder::make('table')
            ...
            ->setPageTips('<p style="color:#f3b760;"><strong><i class="fa fa-fw fa-info-circle"></i> 提示：</strong>ID为1和2的角色不能进行删除、禁用和启用操作。</p>') // 设置页面提示
            ...
            ->addColumns([ // 批量添加列
                ...
                //['access', '是否可登录后台', 'status','',['0'=>'不能','1'=>'可以']],
                ['status', '状态', 'status','',['0'=>'禁用','1'=>'正常']],
                ['right_button', '操作', 'btn']
            ])
            ->addTopButtons('add,enable,disable,delete') // 批量添加顶部按钮
            ->addRightButtons('edit,delete') // 批量添加右侧按钮
            ->replaceRightButton(['id' => 1], '<button class="btn btn-danger btn-xs" type="button" disabled>不可操作</button>') // 修改id为1的按钮
            ->replaceRightButton(['id' => 2], '<button class="btn btn-danger btn-xs" type="button" disabled>不可操作</button>') // 修改id为2的按钮
    ...
    public function add()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (!isset($data['menu_auth'])) {
                ...
                if(UID!=1){
                    foreach ($data['menu_auth'] as $key => $value) {
                        if(db('admin_menu')->where('id',$value)->value('is_show')!=1){
                            unset($data['menu_auth'][$key]);
                        }
                    }
                }
            }
        ...
        if (!$menus) {
            ...
            if(UID!=1){
                $menus = MenuModel::where('module', 'in', $modules)->where('is_show','1')->order('sort,id')->column('id,pid,sort,title,icon');
            }else{
     ...
     * @return mixed
     */
    public function delete($ids = [],$record = [])
    {
        if(UID!=1){
            $ids=(array)$ids;
        ...
        $rt=db('admin_role')->where('id','in',$ids)->delete();
        if($rt!==false){
            db('admin_user')->where('role','in',$ids)->delete();
     ...
     * @return mixed
     */
    public function enable($ids = [],$record = [])
    {
        if(UID!=1){
            $ids=(array)$ids;
            foreach ($ids as $key => $value) {
        ...
        $rt=db('admin_role')->where('id','in',$ids)->update(['status'=>'1']);
        if($rt!==false){
            return $this->success('操作成功');
        }else{
            return $this->error('操作失败，请重试');
     ...
     * @return mixed
     */
    public function disable($ids = [],$record = [])
    {
        if(UID!=1){
            $ids=(array)$ids;
            foreach ($ids as $key => $value) {
        ...
        $rt=db('admin_role')->where('id','in',$ids)->update(['status'=>'0']);
        if($rt!==false){
            return $this->success('操作成功');
        }else{
            return $this->error('操作失败，请重试');
